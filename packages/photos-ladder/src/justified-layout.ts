...
function distribute<T>(
  ...
  // `last` indexes a row that is never empty — `justifiedRows` only ever calls
  // this with the items it just collected — and every width is a number by
  // construction two lines above. The assertions are what the package's
  // `noUncheckedIndexedAccess` asks for and say nothing else.
