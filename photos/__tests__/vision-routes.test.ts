...
describe("GET|PUT /api/vision/config", () => {
  ...
  it("retracts what it published when the toggle goes off", async () => {
    ...
    seed("a", [0]);
    assignUnclusteredFaces(0.45);
    writeVisionConfig({ faces: { enabled: true, threshold: 0.45, publishLabels: true }, scene: { enabled: false }, objects: { enabled: false, threshold: 0.35 } });
  ...
  it("ignores a malformed body rather than clobbering the config", async () => {
    writeVisionConfig({ faces: { enabled: true, threshold: 0.6, publishLabels: false }, scene: { enabled: false }, objects: { enabled: false, threshold: 0.35 } });
...
describe("GET|PUT /api/vision/people", () => {
  ...
  it("rebuilds every cluster, discarding names", async () => {
    seed("a", [0]);
    seed("b", [0.6]);
    writeVisionConfig({ faces: { enabled: true, threshold: 0.9, publishLabels: false }, scene: { enabled: false }, objects: { enabled: false, threshold: 0.35 } });
    assignUnclusteredFaces(0.45);
    const [person] = readPeople();
    person.name = "Alice";
    writePeople([person]);
...
describe("POST /api/vision/scan", () => {
  ...
  it("refuses to start without the models, and names the fetch command", async () => {
    writeVisionConfig({ faces: { enabled: true, threshold: 0.45, publishLabels: false }, scene: { enabled: false }, objects: { enabled: false, threshold: 0.35 } });
    const res = await post({ action: "start" });
    expect(res.status).toBe(409);
