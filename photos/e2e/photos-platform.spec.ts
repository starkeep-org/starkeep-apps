 ...
 * where Photos' selectors change, rather than in core. Core asserts the same
 * platform properties against its own fixture app (`test-apps/probe`), so a
 * deployment without Photos still has them covered.
 *
...
test.beforeAll(async () => {
  ...
  // The first test drives the install consent dialog, which only appears for an
  // app that is not installed. Stated as a precondition this suite enforces
...
test("a photo uploaded in photos appears in Drive; its caption does not", async ({
  page,
}) => {
  await page.goto(photosUrl);
  ...
  const saved = page.waitForResponse(
    (r) =>
      r.url().includes("/api/photos/captions/") &&
...
test("re-uploading the same photo dedups at the platform layer", async ({
  page,
}) => {
  await page.goto(photosUrl);
  await expect(page.getByAltText("sunset.png").first()).toBeVisible({
...
test("uninstall: app data is gone, shared records survive in Drive", async ({
  ...
  await page.goto(driveUrl());
  await expect(
    page.getByRole("row").filter({ hasText: "sunset.png" }).first(),
  ).toBeVisible();
});

test("reinstall re-exposes the shared photos; captions are gone", async ({
  page,
}) => {
  await page.goto(adminUrl());
  const card = photosCard(page);
...
test("a corrupted HMAC secret turns into 401s and a visible error state", async ({
  page,
}) => {
