...
import type { PhotoRecord } from "@/lib/data-server-client";
...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!body?.sourceImageId || !body.cropRect) {
    return Response.json({ error: "sourceImageId and cropRect are required" }, { status: 400 });
  }
  ...
  if (!fileUrlRes.ok) {
    return Response.json({ error: "Failed to get source file URL" }, { status: 502 });
  }
  const { url: sourceUrl } = (await fileUrlRes.json()) as { url: string };

  const sourceRes = await fetch(sourceUrl);
  if (!sourceRes.ok) {
    return Response.json({ error: "Failed to download source image" }, { status: 502 });
  }
  ...
  if (!presignRes.ok) {
    ...
    return Response.json({ error: `Failed to presign upload: ${errBody}` }, { status: 502 });
  }
  ...
  if (!createRes.ok) {
    ...
    return Response.json({ error: `Failed to create crop record: ${errBody}` }, { status: 502 });
  }
  ...
  return Response.json({ image: photoRecordToAppImage(record, photoMeta) }, { status: 201 });
}
