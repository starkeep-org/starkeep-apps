...
export async function PUT(req: Request): Promise<Response> {
  ...
  switch (body.action) {
    case "rename": {
      if (!body.personId || typeof body.name !== "string") {
        return Response.json({ error: "personId and name are required" }, { status: 400 });
      }
    ...
    case "split": {
      if (!Array.isArray(body.faces) || body.faces.length === 0) {
        return Response.json({ error: "faces must be a non-empty array" }, { status: 400 });
      }
      if (splitFacesToNewPerson(body.faces) === null) {
        return Response.json({ error: "none of those faces exist" }, { status: 404 });
      }
      break;
    }
    case "recluster": {
  ...
  return GET().then(async (res) => {
    ...
    return Response.json(warning ? { ...payload, warning } : payload);
  });
}
